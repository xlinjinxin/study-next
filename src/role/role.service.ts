...
export class RoleService extends Repository<Role> {
  ...
  public createRole = async (createRoleDto: CreateRoleDto) => {
    ...
    // }
      ...
      if(role){
        role.permissions = createRoleDto.permissions?.map(item => { return { id: item } })
        console.log(role, 123123)
        return this.repository.save(role)
      }else{
        createRoleDto.permissions = createRoleDto.permissions?.map(item => { return { id: item } })||[]
  ...
  public queryList<T, U>(
    ...
    return this.repository
      .createQueryBuilder('item').leftJoinAndSelect("item.permissions", "permission")
